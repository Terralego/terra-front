...
export const Story = ({ map, story: { beforeEach = [], slides }, setLegends, translate }) => {
  ...
  const nextStep = useCallback(() => {
    setStep(prevStep => (prevStep === slidesLength
      ? 0
      : prevStep + 1));
  }, [slidesLength]);
  ...
  const displayButtons = slidesLength > 0;
    ...
    <div className="storytelling">
      ...
      {displayButtons && (
        <div className={classnames('storytelling__buttons', { 'storytelling__buttons--justified': displayPrevButton })}>
          ...
          <Button
            rightIcon={step !== slidesLength && 'chevron-right'}
            icon={step === slidesLength && 'step-backward'}
            large
